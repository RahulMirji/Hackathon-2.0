...
export default function MCQ3Page() {
  ...
  useEffect(() => {
    const initializeSection = async () => {
      try {
        ...
        console.error("❌ [MCQ3] Failed to initialize section:", error)
    ...
    const loadQuestions = async () => {
      ...
      if (cachedQuestions && cachedQuestions.length > 0) {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`📋 [MCQ3] Using cached questions for ${section}:`, cachedQuestions.length)
        }
      ...
      try {
        if (process.env.NODE_ENV !== 'production') {
          console.log(`📡 [MCQ3] Loading questions for ${section}...`)
        }
        const result = await getOrLoadSectionQuestions(section)
        setQuestions(result.questions)
        setIsLoading(false)
      } catch (error) {
        console.error(`❌ [MCQ3] Failed to load questions:`, error)
        setIsLoading(false)
        setQuestions([])
  ...
  }, [sessionInitialized, startQuestionTimer])
  ...
  if (isLoading || questions.length === 0) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <Card className="p-8">
  ...
  return (
    <main className="exam-container min-h-screen bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-900 dark:to-gray-800">
      {/* Header */}
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push("/exam/mcq2")}
              className="gap-2 hover:bg-gray-100 dark:hover:bg-gray-800"
              title="Go back to MCQ 2"
            >
              <ChevronLeft className="h-5 w-5" />
            </Button>
            <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-green-600 to-teal-600 flex items-center justify-center shadow-md">
              <FileText className="h-5 w-5 text-white" />
            </div>
            <div>
              <h1 className="font-semibold text-lg">{sectionInfo.title}</h1>
              <p className="text-xs text-muted-foreground">MCQ 3 Exam</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button onClick={handleSubmit} className="gap-2 bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white shadow-lg">
          ...
          <div className="h-full overflow-hidden pr-2">
            <div className="space-y-3 h-full flex flex-col">
              ...
              <Card className="exam-card shadow-lg border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex-1 overflow-hidden">
                <div className="bg-gradient-to-r from-green-600 to-teal-600 text-white px-4 py-2 -mx-6 -mt-6 mb-3 rounded-t-lg">
          ...
          <div className="h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent pr-2">
            <div className="space-y-4">
              ...
              <ExamQuestions
                ...
                nextSectionRoute="/exam/coding"
                onNavigateToNextSection={() => router.push("/exam/coding")}
              />
            </div>
          </div>
